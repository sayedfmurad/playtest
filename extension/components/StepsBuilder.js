(function(){
  ...
  function StepsBuilder({ onStatusChange }){
    ...
    async function pickSelector(id) {
      const res = await sendToTab({ type: 'start_picker' });
      if (res && res.ok && res.selector) updateStep(id, { selector: res.selector });
    }

    async function pickPosition(id) {
      const res = await sendToTab({ type: 'start_position_picker' });
  ...
  window.StepsBuilder = StepsBuilder;
})();
